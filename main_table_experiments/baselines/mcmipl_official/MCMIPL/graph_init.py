...
def main():
    ...
    parser.add_argument('--data_name', type=str, default=BOOK, choices=[LAST_FM_STAR, YELP_STAR, BOOK, MOVIE],
                        help='One of {LAST_FM_STAR, BOOK, YELP_STAR, MOVIE}.')
    ...
    if args.data_name=='BOOK':
        ...
        print('Create LAST_FM_STAR graph from raw data...')
        kg = LastFmGraph()
        save_kg(args.data_name, kg)
        print('Save LAST_FM_STAR graph successfully!')
        print('Create LAST_FM_STAR dataset...')
        dataset = LastFmStarDataset()
        save_dataset(args.data_name, dataset)
        print('Save LAST_FM_STAR dataset successfully!')
        return
